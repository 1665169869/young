...
class young:
    def changMCode(self):
        ...
        返回二进制数据
        """
        unixTime = int(round(time() * 1000))
        url = "http://{0}/common/image_code.jsp?time={1}".format(self.ip, str(unixTime))
    ...
    def login(self, username, password, code, wlanUserIP, wlanAcIP="183.56.22.50"):
        """登录账户校园网
        username:账号
        password:密码
        wlanUserIP:本地用户IP
        code:验证码，请用changCode()获取
        wlanAcIP:验证端IP
        """
        ...
        if response.status_code == 200:
            try:
                self.headers['cookie'] = self.JESSIONID + ";signature=" + response.cookies['signature']
                self.headers['cookie'] += ";loginUser=" + response.cookies['loginUser'] 
            except:
                pass
    ...
    def __init__(self, ip='125.88.59.131:10001'):
        """
        首次访问获取JSESSIONID的cookie
        用于图片验证
        """
        self.ip = ip
        url = "http://{0}/ajax/login".format(self.ip)
